...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # blanck after comma, means choose all the columns
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") # | means or
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # %in% means include
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #, equasl to & here
...
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999) # 1999 or “1999” both work here
...
NTL.phys.data.1990_1999 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> ANSWER: It's not understandable; you have to check and look at the rows you need; 
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) # : and , to select
...
NTL.phys.data.processed <- 
  NTL.phys.data %>% # %>% -> then, continue the line, and can add more functions (pipes)
...
# one dot means go down, two means go up
```
