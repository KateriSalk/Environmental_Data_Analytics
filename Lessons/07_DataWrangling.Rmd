...
#choose lake names that have both paul lake and peter lake
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")  
#choose lake names that exclude the following items, use != and &
...
class(NTL.phys.data$year4) # it is integer
...
NTL.phys.data.Tuesdaylake1990.1999 <- filter(NTL.phys.data, year4 >= 1990 & 
                                               year4 <= 1999 &
...
# 7/16/1998, East Long Lake, 0.5 m depth
...
# this is chooseing columns with lakename, and from sample data to temperature_C
...
```{r}
#added a new column that has done something on a previous column
...
```{r}
#why do we choose false for rolnames?
