...
vector1[3] #give you the third elemnt of vector1 -5.3 

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # create matrix: 1:20 - show numbers 1 to 20
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #byrow=True, horizontally first; the default is vertically
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #create names for the rows and columns
matrix3
matrix3['uno',]

matrix1[4, ] #pull the submatrix of the fourth row
matrix1[ , 3] #pull the submatrix of the third column
matrix1[c(12, 14)] #pull the elements no.12 and 14, they happen to be 12 and 14 in matrix1, but will be 7 and 15 in matrix 2 becase the sequence in R moves by vertically first
matrix1[c(12:14)] #pull the elements no. 12 to 14
matrix1[2:4, 1:3] #pull the submatrix of row 2 to 4 and column 1 to 3

cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # define at least one of row number or column number; if cells does not 4 numbers, it will fill in the first a few number in sequence atuomatically, but it will generate warning message
matrix4

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1 # lists are used for elemtens for different types; if not define a name, it will just call it [[2]]
list1[[2]]
list1[2] 
...
dataframe1[1:2] # pull collums 1 and 2; [1:2,] - pull rows 1 and 2
dataframe1[c("ID","Passed")] # pull column "ID" and "Passed"
dataframe1$Color # pull specific column
