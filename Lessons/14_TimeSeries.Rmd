...
# there is a trend over time according to this test (p<0.0001), Z is positive, positive trend - time passes, value increasess 
```
...
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  geom_vline(xintercept = as.Date("1993/5/20"), origin="1970-01-01", color="#253494", lty = 2)+ #default date origin 1970-01-01
  geom_vline(xintercept = as.Date("1998/1/1"), color="#253494", lty = 2)+
  scale_color_manual(values = c("#7fcdbb", "#253494"))
...
> ANSWER: Piecewise constant interpolation | to determine linear and spline, check pvalue, but linear is more reliable for environmental data in most cases
...
# Create a time series object, monthly-> frequency=12, daily, f=365
...
#seasonal change inside this season
```
