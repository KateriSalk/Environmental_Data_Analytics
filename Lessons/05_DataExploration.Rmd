...
It might be different when you switch the laptop
...
# Another option is to choose with your browser, a window will pop up
# read.csv(file.choose())
# Why is it not as good as coding the path? - Not reproducible; because people might not choose the right one
...
str(USGS.flow.data)  #tell you the structure
dim(USGS.flow.data)  #tell you the dimension, which is the same as the first line of str
length(USGS.flow.data)

head(USGS.flow.data)
head(USGS.flow.data, 10)  #give us the first 10 rows, default is 6
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)]  #note: using the [] bracket; this means in row 30000 to 30005, gives me what's in column 3, 8 and 14
...
summary(USGS.flow.data$discharge.mean)  #the output shows you mean, max, min, how many NA's
summary(USGS.flow.data$discharge.mean.approval) #look how it changes, it doesn't have mean because this column is factor not numerical, so be careful which column you are pulling from
...
> ANSWER: They automatically become NA
