...
format(today-1)  #you can also add or substract days from it
format(today, format = "%m-%d, %y")
format(today, format = "%B %d, %Y")
format(today, format = "%m %d, %a, %Y")

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #note that R took all the 1928 to 2028
```
...
```{r}
# first remove the "20" in front of the number
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# create a function, if it is > 181231, which means that if it's greater put 19, if smaller than 181231 then put 20; 

create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d) # paste: it will take the two things inside and put them together; 
       })
#
...
> Answer: Remove NAs - true NA (people's answer is actually NA = not available) and missing data

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # if there's any NA in that row, it removes the whole row
dim(USGS.flow.data)   
