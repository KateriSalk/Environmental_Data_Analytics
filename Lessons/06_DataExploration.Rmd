...
```{r}
# change it to the format of year-month-day sequence number, just use format instead of as.date -- Xin 01292019
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# create a function to identify year exceed this year -- Xin 01292019
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
# add the correct year to the date -- Xin 01292019
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

# format the year-month-date as data format -- Xin 01292019
...
> Answer: When NA is one of the answers, it is favorable; when it indicates missing data, we need to remove them. Certain ways of stats mdoels do not allow NAs. -- Xin 01292019

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #delete the complete row if there is NA in one cell  -- Xin 01292019
dim(USGS.flow.data)
dim(USGS.flow.data.complete) # dim - dimension
...
```{r, fig.height = 3, fig.width = 4}
# create three geom_freqpoly data series
ggplot(USGS.flow.data.complete) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) + # color black by default
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "blue") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50, color = "red") +
  scale_x_continuous(limits = c(0, 10))

# color this plot by its stats status
ggplot(USGS.flow.data.complete) +
...
# group by the increment of gage.height difference of 1, the higer the river, the more water will flow through
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# the number of certain range of data, the wider indicates the more data
ggplot(USGS.flow.data.complete) +
