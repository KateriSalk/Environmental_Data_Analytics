...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #box around the box and white bg
print(O3plot2)

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() # no box outside compared to bw
print(O3plot3)
...
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), 
        legend.position = "top") #alternative: legend.position = c(1,1) + legend.justification = c(1,1))
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  ylab(expression("O"[3]* " AQI Value")) + # no definition for xaxis, it will take the Date (variable name) as the xaxis lable
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #angel - change xaxis label angle, hjust - move horizontally, move to the left
print(O3plot7)  
...
library(viridis)
library(RColorBrewer) # colorBrewer online can help show the cloar pattern
library(colormap)
...
NvsP2 <-
  ...
  #xlab("Totoal Phosphorus (Microgram)(\U003BCg/L)") +
  #ylab("Totoal Nitrogen (Microgram)") +
  labs(x ="", y="", fill ="Depth, m", shape = "Lake")+
