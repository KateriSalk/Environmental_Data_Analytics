...
<notes for yourself that will not be shown to others>

- Code, output, and test/notes together in one document
+ knit to useful formats (pdf, html, docx)
* make sure there has to be a return before the bullet points
- legible codes + output
- Git friendly
- Reproducible
- Updating capabilities
- Focus on output and conclusions, not code (flexible formatting)
- simple syntax and autoreferecing
...
```{r, echo=FALSE}
nu_summary <- nutrients_sel %>%
  group_by(lakename) %>%
  summarise(tn_mean = mean (tn_ug),
            tn_min = min (tn_ug),
            tn_max = max (tn_ug),
            tn_sd = sd (tn_ug))

tp_summary <- nutrients_sel %>%
  group_by(lakename) %>%
  summarise(tn_mean = mean (tp_ug),
            tn_min = min (tp_ug),
            tn_max = max (tp_ug),
            tn_sd = sd (tp_ug))
```
...
```{r}
kable(nu_summary, caption="Total N")
kable(tp_summary, caption="Total p")
