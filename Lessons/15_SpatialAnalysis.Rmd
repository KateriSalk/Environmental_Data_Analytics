...
3 things:
Geometry: shape
Attributes: what it is, description
Coordinate graphic system
...
ggplot(EPAair_PM_avg, aes(x=SITE_LONGITUDE, y=SITE_LATITUDE)) + 
  geom_point(aes(color=meanPM), size=4) + coord_equal() #This helps you put it into coordinate kind of scale, so with this you will see the shape of an actual map
```
