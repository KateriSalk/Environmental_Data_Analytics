...
class(NTL.phys.data.PeterPaul$sampledate) #usually R will not load date as date, but as factor
...
NTL.nutrient.data.PeterPaul <- filter(NTL.nutrient.data, lakename == "Paul Lake" | lakename == "Peter Lake") # filter selects rows, select selects columns
...
summary(NTL.nutrient.data.PeterPaul$comments) # We have filtered out the other lakes, but it still keep the information e.g. comment for other lakes here
...
write.csv(NTL.nutrient.data.PeterPaul, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaul_Processed.csv") #if there is a file has same name, it just gonna overwrite it
...
#when using select, can put '-' before the variable to remove it when selecting
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.skinny, "nutrient", "concentration", tn_ug:po4) #':' means through, gather function gather two datafrem by the selected columns toghter, and the total rows increase, multily the original dataset down
...
#warning message: lakename will different levels
...
library(lubridate)
#there is function date from base package, and from this point, it will use the funciton from lubridate
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#you can run any summary option at this point
