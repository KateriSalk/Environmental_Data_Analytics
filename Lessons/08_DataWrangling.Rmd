...
# filter select rows, and select select columns
...
test <- droplevels(NTL.nutrient.data.PeterPaul) # only keep the peter and paul 

# Save processed nutrient file
# rol names = false coz we don't want to save the most left column that's from 1 to ...
...
# we can just add "-" in front of the columns we wanna remove
NTL.phys.data.PeterPaul.skinny <- select(NTL.phys.data.PeterPaul, 
                                         -lakename, -daynum, -year4, -sampledate:irradianceDeck)
...
# Remove the data that are missing in nutrient data
...
NTL.nutrient.data.PeterPaul.spread <- spread(PeterPaul.nutrients.gathered, nutrient, concentration)

# Split components of cells into multiple columns
# Opposite of 'separate' is 'unite'
# this separate will NOT retain the original column, if you want to, you might need to mutate instead
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>%
  group_by(lakename) %>%
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #it returns a situation where it remove even just one of these columns have na value 
