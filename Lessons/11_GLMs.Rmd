...
O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") #regular t-test, two side, by idneticating less, it fits our research question
O3.onesample
# low p value, reject H0; t score, the extreamest, the greater possibility

```

What information does the output give us? How might we report this information in a report?

> ANWSER: Ozone AQI values in 2017-2018 were significantlly less than 50 (one sample t-test; t=-41.9, df =1084, p<0.0001)
...
# Low P value - Siginificant difference in the means in the two groups
# y= Blackstone(38.48) +Bryson(35.18)+delta

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Site.Name)
summary(O3.twosample2)
#intercept is the first factor, the Blackstone
```
...
O3.twosample.wilcox
#tell us there is differnece, but not clear where the difference is 
```
...
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) # two to test, barlett
#there are not significantly differently
...
#TN = 309.39, Paul lake, depth id 1
