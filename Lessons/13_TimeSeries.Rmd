...
PeterPaul.summertemp <- 
  ...
  na.exclude() #autocorrelation doesn't work with NAs
...
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     temperature_C ~ sampledate * lakename, #fixed effect ANCOVA
                     random = ~1|Week) #specify random effects
TempTest.auto 
# 1.58 sd, 1.94 residual, 14 weeks (groups)
...
cld(TempTest.posthoc, alpha = 0.05, Letters = letters, adjust = "tukey") #both group a, no signifiant difference
