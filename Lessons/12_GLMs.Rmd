...
```{r}
#Make sure explanatory variable(x) is on the right, dependent variable(y) is on the left
...
# irrdiance = 487 - 96*(depth) + error for each meter we go down in the water, our irridiance is gonna go down 96 
...
> ANSWER: general summary: irridiance is negatively associated with depth (linear regression; R2 = 0.31, df = 15449, p < 0.0001)
...
# log(irridiance) = 6.2 - 0.74*(depth) + error
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") + #if you wanna put the line above the point, then put geom_smooth() after you put geom_point
  scale_y_log10() + #can just plot the y value to log scale
  geom_point()
...
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(alpha = 0.5) + #this makes the points 50% transparent
    scale_y_log10() +
    labs(x = "depth (m)", y = "Water Irridiance (units)") +
    scale_x_continuous(breaks = c(0, 2, 4, 6, 8, 10, 12)) +
    geom_smooth(method = 'lm', se = FALSE, color = "red") #se = FALSE can remove the standard deviation interval
print(irradiancebydepth2)

# Make the graph attractive
# you can put 0 at 0, make them line up
...
summary(TPregression) # we put + between multiple variables
# TP = 6 + 1.5 * depth + 0.94 * dissolved oxygen + error (both depth and DO are significant predictors)
# however, we have an R square of 0.08: this means that our stats only explain 8% of variance in the data
# at the surface when we have 0 oxygen we have a prediction of 6 TP
...
TPplot <- ggplot(PeterPaul.chem.nutrients, 
  ...
  xlim(0, 20)
print(TPplot)
# this shows that our linear model is not a good model to show the relationship
```
### Correlation Plots
...
library(corrplot)
PeterPaulnutrients <- #the corrplot needs a full dataset; can't have any NAs
  PeterPaul.chem.nutrients %>%
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse") #if not elipse, it's only gonna show the color intensity; elipse shows a direcional relationship; nitrate and total nitrogen has a strong positive relationship
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #this one show you both the number and the shape
# Why is it problematic to throw in variables and see what happen?
# there is a chance to have some correlation between the variables, but they might not make sense at all 
...
# we just chose what we think will be a good variable based on what we know
step(TPAIC)
#the lower the AIC number the better, for this one it's 884.92; but it's only relevant to each other in the dataset
#basically, it is to see removing which variable will result in the most decrease in AIC
#for example, removing dissolved oxygen will result in an AIC of 882.96, removing temperature will result in an AIC of 903, so you would remove dissolved oxygen instead of temperature
TPmodel <- lm(data = PeterPaul.naomit, tp_ug ~ temperature_C + tn_ug)
#This is the best model we get from AIC and we can run this to better see its information
summary(TPmodel)
...
# what's effect of lake name and what's the effect of depth?
summary(TNancova.main)
# we predicting total nitrogen
# the intercept is 353, this means paul lake at depth 0 will have 353 ug/L tn
# then for peter lake at depth 0 will have 135 MORE tn 
# then depth is another main effect, we see a 9.7 decrease in tn as it goes 1m deeper

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)
# for peter lake: we decrease in 48.42 for depth; but for paul lake we inrease in 19.87 for depth
