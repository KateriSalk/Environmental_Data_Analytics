...
>R -1 to 1, close to 0, no strong corrolation, R^2, 0 to 1
...
#R^2 - how much data explained
```
Question: How would you report the results of this test (overall findings and report of statistical output)?

> ANSWER: The irradiance is negatively correlated with depth (maybe more explanation of coefficients)
> (linera Regression; R2=0.31, df=15449, p<0.0001)
...
ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(alpha=0.2) + #80% transparent
    scale_y_log10() +
    scale_x_continuous(breaks = c(0, 2, 4, 6, 8, 10, 12))+
    #geom_text() #add annotation
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit() #to plot has to be no NA in dataset
...
# 20(Pual, depth), if it is peter -48.42
