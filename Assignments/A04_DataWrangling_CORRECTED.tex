...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#1 check the working directory, load the correct package, as well as raw data files}
...
\CommentTok{#2 Understand what's in the dataset and basic information}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#3 Make sure R recognize the date in the datasets are date}
...
\CommentTok{#4 Select only the following columns: Date, DAILY_AQI_VALUE, Site.Name, AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, SITE_LONGITUDE}
\NormalTok{O3_}\DecValTok{2017}\NormalTok{ <-}\StringTok{ }\KeywordTok{select}\NormalTok{(O3_}\DecValTok{2017}\NormalTok{, Date, DAILY_AQI_VALUE, Site.Name, AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, SITE_LONGITUDE)}
\NormalTok{O3_}\DecValTok{2018}\NormalTok{ <-}\StringTok{ }\KeywordTok{select}\NormalTok{(O3_}\DecValTok{2018}\NormalTok{, Date, DAILY_AQI_VALUE, Site.Name, AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, SITE_LONGITUDE)}
\NormalTok{pm25_}\DecValTok{2017}\NormalTok{ <-}\StringTok{ }\KeywordTok{select}\NormalTok{(pm25_}\DecValTok{2017}\NormalTok{, Date, DAILY_AQI_VALUE, Site.Name, AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, SITE_LONGITUDE)}
\NormalTok{pm25_}\DecValTok{2018}\NormalTok{ <-}\StringTok{ }\KeywordTok{select}\NormalTok{(pm25_}\DecValTok{2018}\NormalTok{, Date, DAILY_AQI_VALUE, Site.Name, AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, SITE_LONGITUDE)}

\CommentTok{#5 For the PM2.5 datasets, fill all cells in AQS_PARAMETER_DESC with "PM2.5" (all cells in this column should be identical)}
\NormalTok{pm25_}\DecValTok{2017}\OperatorTok{$}\NormalTok{AQS_PARAMETER_DESC <-}\StringTok{ "PM2.5"}
\NormalTok{pm25_}\DecValTok{2018}\OperatorTok{$}\NormalTok{AQS_PARAMETER_DESC <-}\StringTok{ "PM2.5"}

\CommentTok{#6 Export all four air data as csv in the processed data folder}
\KeywordTok{write.csv}\NormalTok{(O3_}\DecValTok{2017}\NormalTok{, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }\StringTok{"./Data/Processed/EPAair_O3_2017_processed.csv"}\NormalTok{)}
\KeywordTok{write.csv}\NormalTok{(O3_}\DecValTok{2018}\NormalTok{, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }\StringTok{"./Data/Processed/EPAair_O3_2018_processed.csv"}\NormalTok{)}
\KeywordTok{write.csv}\NormalTok{(pm25_}\DecValTok{2017}\NormalTok{, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }\StringTok{"./Data/Processed/EPAair_pm25_2017_processed.csv"}\NormalTok{)}
\KeywordTok{write.csv}\NormalTok{(pm25_}\DecValTok{2018}\NormalTok{, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }\StringTok{"./Data/Processed/EPAair_pm25_2018_processed.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#7 Combine all four datasets into one}
\NormalTok{o3_pm25_}\DecValTok{1718}\NormalTok{ <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(O3_}\DecValTok{2017}\NormalTok{, O3_}\DecValTok{2018}\NormalTok{, pm25_}\DecValTok{2017}\NormalTok{, pm25_}\DecValTok{2018}\NormalTok{)}

\CommentTok{#8 Select only 3 sites: Blackstone, Bryson City, Triple Oak; and add two new columns of their month and year}
\NormalTok{o3_pm25_}\DecValTok{1718}\NormalTok{ <-}\StringTok{ }
\StringTok{  }\NormalTok{o3_pm25_}\DecValTok{1718} \OperatorTok{%>%}
...
\CommentTok{#9 Separate the column that is currently filled with ozone and PM2.5 into two separate columns; basically you grabbed the values under the two columns}
\NormalTok{o3_pm25_}\FloatTok{1718.}\NormalTok{spread <-}\StringTok{ }\KeywordTok{spread}\NormalTok{(o3_pm25_}\DecValTok{1718}\NormalTok{, AQS_PARAMETER_DESC, DAILY_AQI_VALUE)}

\CommentTok{#10 Look up the dimension of the new processed data}
\KeywordTok{dim}\NormalTok{(o3_pm25_}\FloatTok{1718.}\NormalTok{spread)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#11 Export the data to csv file}
\KeywordTok{write.csv}\NormalTok{(o3_pm25_}\FloatTok{1718.}\NormalTok{spread, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{,    }\StringTok{"./Data/Processed/EPAair_O3_PM25_NC1718_Processed.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#12a Create a summary table of mean AQI values for O3 and PM2.5 by month}
\NormalTok{o3_pm25_}\FloatTok{1718.}\NormalTok{summary.month <-}
\StringTok{  }\NormalTok{o3_pm25_}\FloatTok{1718.}\NormalTok{spread }\OperatorTok{%>%}
...
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{mean_o3 =} \KeywordTok{mean}\NormalTok{(Ozone),}
            \DataTypeTok{mean_pm2.5 =} \KeywordTok{mean}\NormalTok{(PM2.}\DecValTok{5}\NormalTok{))}

\CommentTok{#12b Create a summary table of the mean, minimum, and maximum AQI values of O3 and PM2.5 for each site}
\NormalTok{o3_}\FloatTok{1718.}\NormalTok{summary.site <-}
\StringTok{  }\NormalTok{o3_pm25_}\FloatTok{1718.}\NormalTok{spread }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(Site.Name) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(}\OperatorTok{!}\KeywordTok{is.na}\NormalTok{(Ozone)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{mean_o3 =} \KeywordTok{mean}\NormalTok{(Ozone),}
            \DataTypeTok{min_o3 =} \KeywordTok{min}\NormalTok{(Ozone),}
            \DataTypeTok{max_o3 =} \KeywordTok{max}\NormalTok{(Ozone))}
          
\NormalTok{pm25_}\FloatTok{1718.}\NormalTok{summary.site <-}
\StringTok{  }\NormalTok{o3_pm25_}\FloatTok{1718.}\NormalTok{spread }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(Site.Name) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(}\OperatorTok{!}\KeywordTok{is.na}\NormalTok{(PM2.}\DecValTok{5}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{mean_pm2.5 =} \KeywordTok{mean}\NormalTok{(PM2.}\DecValTok{5}\NormalTok{),}
...
\NormalTok{o3pm25_}\FloatTok{1718.}\NormalTok{summary.site <-}\StringTok{ }\KeywordTok{full_join}\NormalTok{(o3_}\FloatTok{1718.}\NormalTok{summary.site, pm25_}\FloatTok{1718.}\NormalTok{summary.site)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## Joining, by = "Site.Name"
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#13 View the summary tables}
\KeywordTok{view}\NormalTok{(o3_pm25_}\FloatTok{1718.}\NormalTok{summary.month)}
\KeywordTok{view}\NormalTok{(o3pm25_}\FloatTok{1718.}\NormalTok{summary.site)}
\end{Highlighting}
\end{Shaded}
